  ...
  */
  class DateTimeValueLib {
    ...
    */
	...
	static function toNormalDate($fecha,$separador="/"){
	...
	//Convierte DateTIme de mysql a normal
	static function toNormalDateAndTime($fecha,$separador="/"){
		$hora =  split( " ", $fecha);
		$lahora = $hora[1];
		ereg( "([0-9]{2,4})-([0-9]{1,2})-([0-9]{1,2})", $fecha, $mifecha);
		$lafecha=$mifecha[3].$separador.$mifecha[2].$separador.$mifecha[1];
	...
	static function toMySqlDate($fecha){	
	...
	//Convierte fecha de normal a DateTIme mysql
	static function toMySqlDateAndTime($fecha){
		$hora =  split( " ", $fecha);
		$lahora = $hora[1];
		ereg( "([0-9]{1,2})/([0-9]{1,2})/([0-9]{2,4})", $fecha, $mifecha);	
		$lafecha=$mifecha[3]."-".$mifecha[2]."-".$mifecha[1];
	...
	//Convierte fecha de mysql para el formato del calendario
	static function toCalendarDate($fecha){
		$hora =  split( " ", $fecha);
		$lahora = $hora[1];
		$fecha=$hora[0];
		$lafecha=$fecha."T".$lahora.".000+03:00";
		return $lafecha;
	}
